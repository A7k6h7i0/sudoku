      ...
      <div className={`${darkMode ? 'bg-gray-800 shadow-lg shadow-black/20' : 'bg-gradient-to-br from-cyan-50 via-white to-blue-50 border border-cyan-100 shadow-lg shadow-cyan-200/40'} rounded-2xl p-2.5 md:p-3`}>
        <div className="flex justify-center gap-2.5 md:gap-3">
          ...
          <button
            ...
            disabled={!canUndo}
            className={`
              flex flex-col items-center gap-1 w-14 md:w-14
            ...
            `}
          >
            <i className={`ri-arrow-go-back-line text-[34px] md:text-[28px] ${canUndo ? (darkMode ? 'text-gray-300' : 'text-gray-700') : (darkMode ? 'text-gray-600' : 'text-gray-300')}`}></i>
            <span className={`text-[1.2rem] md:text-base font-medium ${canUndo ? (darkMode ? 'text-gray-300' : 'text-gray-700') : (darkMode ? 'text-gray-600' : 'text-gray-300')}`}>Undo</span>
          </button>
          ...
          <button
            ...
            `}
          >
            <i className={`ri-arrow-go-forward-line text-[34px] md:text-[28px] ${canRedo ? (darkMode ? 'text-gray-300' : 'text-gray-700') : (darkMode ? 'text-gray-600' : 'text-gray-300')}`}></i>
            <span className={`text-[1.2rem] md:text-base font-medium ${canRedo ? (darkMode ? 'text-gray-300' : 'text-gray-700') : (darkMode ? 'text-gray-600' : 'text-gray-300')}`}>Redo</span>
          </button>

          {/* Erase/Clear Button */}
          <button
            onClick={onClear}
            className={`flex flex-col items-center gap-1 w-14 md:w-14 ${darkMode ? 'text-gray-300 hover:text-[#3B5BDB]' : 'text-gray-700 hover:text-[#3B5BDB]'} transition-all duration-200 active:scale-95`}
          >
            <i className={`ri-eraser-line text-[34px] md:text-[28px] ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}></i>
            <span className={`text-[1.2rem] md:text-base font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Erase</span>
          </button>
          ...
          <button
            ...
            `}
          >
            <i className={`ri-pencil-line text-[34px] md:text-[28px] ${notesMode ? 'text-[#3B5BDB]' : (darkMode ? 'text-gray-300' : 'text-gray-700')}`}></i>
            <span className={`text-[1.2rem] md:text-base font-medium ${notesMode ? 'text-[#3B5BDB]' : (darkMode ? 'text-gray-300' : 'text-gray-700')}`}>Pencil</span>
          </button>
          ...
          >
            <div className="relative">
              <i className={`ri-lightbulb-line text-[34px] md:text-[28px] ${hintsLeft === 0 ? (darkMode ? 'text-gray-600' : 'text-gray-300') : (darkMode ? 'text-gray-300' : 'text-gray-700')}`}></i>
              {hintsLeft > 0 && (
                <span className="absolute -top-1 -right-2 min-w-6 h-6 px-1 bg-[#3B5BDB] text-white text-[12px] font-semibold rounded-full flex items-center justify-center">
                  {hintsLeft}
                </span>
              )}
            </div>
            <span className={`text-[1.2rem] md:text-base font-medium ${hintsLeft === 0 ? (darkMode ? 'text-gray-600' : 'text-gray-300') : (darkMode ? 'text-gray-300' : 'text-gray-700')}`}>Hint</span>
          </button>
        </div>
      </div>
          ...
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => {
            ...
            return (
              <button
                ...
                disabled={isDisabled}
                className={`
                  w-full aspect-square mx-auto rounded-xl flex items-center justify-center
                  transition-all duration-200 active:scale-95
                  ${isDisabled 
              ...
              >
                <span 
                  className={`text-[2.2rem] md:text-[30px] leading-none font-bold ${isDisabled ? 'text-gray-500' : 'text-[#0E4FA8]'}`}
                >
                  {num}
                </span>
              </button>
